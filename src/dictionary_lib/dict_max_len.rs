...
impl Serialize for DictMaxLen {
    /// Serializes the dictionary into a compact JSON-compatible object.
    ///
    /// This custom implementation overrides Serde’s derived `Serialize`
    /// behavior to:
    ///
    /// - Convert keys from `Box<[char]>` to `String` for JSON output.
    /// - Convert `starter_cap` keys from `char` to single-character `String`s.
    /// - Serialize only the **persisted** fields:
    ///   - `"map"` — phrase → replacement string mapping.
    ///   - `"max_len"` — global maximum phrase length (characters).
    ///   - `"starter_cap"` — per-starter maximum phrase length (characters).
    ///
    /// Runtime-only fields ([`first_len_mask64`], [`first_char_max_len`])
    /// are intentionally **not** serialized, as they can be regenerated by
    /// [`populate_starter_indexes`](#method.populate_starter_indexes).
    ///
    /// # Output format
    /// ```json
    /// {
    ...
    where
        S: Serializer,
    {
        // Wrap &self.map to serialize keys as Strings
        ...
        // Wrap &self.starter_cap to serialize keys as Strings
...
impl<'de> Deserialize<'de> for DictMaxLen {
    /// Deserializes a [`DictMaxLen`] from either the current object format or a legacy array-of-pairs format.
    ///
    /// # Accepted formats
    ///
    /// ## Current format (object-based)
    /// ```json
    /// {
    ///   "map": {
    ///     "你好": "您好",
    ...
    ///
    /// # Behavior
    /// - `map` keys are converted from `String` → `Box<[char]>` for zero-allocation lookups.
    /// - `map` values are converted to `Box<str>`.
    /// - `starter_cap` keys **must** be single-character strings; they are converted to `char`.
    /// - `max_len` is used if provided; otherwise, it is recomputed as the maximum key length in Rust `char` units.
    /// - Runtime-only fields ([`first_len_mask64`], [`first_char_max_len`]) are always initialized empty and can be built later with [`populate_starter_indexes`](#method.populate_starter_indexes).
    ///
    /// # Errors
    /// - If a `starter_cap` key is empty or contains more than one character, an error is returned.
    ///
    /// # Example
    /// ```
    /// use opencc_fmmseg::dictionary_lib::DictMaxLen;
    ...
    /// }"#;
    ///
    /// let dict: DictMaxLen = serde_json::from_str(json).unwrap();
    /// assert_eq!(dict.max_len, 2);
    /// assert!(dict.starter_cap.contains_key(&'你'));
    /// ```
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        // Accept both object and array-of-pairs for "map"
        #[derive(Deserialize)]
        #[serde(untagged)]
        enum MapRepr {
        ...
        // Use provided max_len or recompute from keys
